...
/**
 * Diagnostic variant (STO-15 round 2, protocol by local Codex): 'A' is the
 * round-1 prompt byte for byte; 'B' removes ONLY the 5–6 profile's prescription
 * «одна главная трудность, к которой герой подступается два-три раза
 * по-разному» — one variable, nothing else changes.
 */
export type AuthorVariant = 'A' | 'B';

const AGE_PROFILE_B_5_6 = `Слушателю 5–6 лет. Герой и два-три участника. Предложения разной длины, много живого диалога; причины и следствия ясны из событий. Часть детей этого возраста уже читает сама — пусть речь персонажей нельзя перепутать, а фразы не требуют объяснений.`;

const ageProfile = (ageBand: AgeBand, variant: AuthorVariant): string =>
  variant === 'B' && ageBand === '5-6' ? AGE_PROFILE_B_5_6 : AGE_PROFILE[ageBand];
...
export const buildAuthorSystem = (ageBand: AgeBand, variant: AuthorVariant = 'A'): string => {
