 ...
 *
 * Runs an eval set (goal × age × mode combos spanning both arcs, both modes,
 * and fallback-exemplar goals) through the text-only pipeline (no images/PDF/DB),
 * prints a per-run table + per-criterion aggregates + deterministic prose
 * metrics, and optionally writes JSON for diffing between runs.
 *
