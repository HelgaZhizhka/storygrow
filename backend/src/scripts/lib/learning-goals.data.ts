/**
 * The curated learning-goal catalogue — one source for the DB seed
 * (`seed-learning-goals.ts`) and for DB-free research harnesses that need a
 * goal's description and arc without a database (`eval:whole-story`, STO-15).
 */
export interface LearningGoalSeed {
...
export const LEARNING_GOALS: readonly LearningGoalSeed[] = [
  {
    title: 'Дружба',
...
export const findLearningGoal = (title: string): LearningGoalSeed | undefined =>
  LEARNING_GOALS.find((g) => g.title === title);
