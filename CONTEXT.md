...
Since #367 the judge also sees the story's fixed world (places, characters, props) and reports an **informational** `pictureConsistency` score — the text names only what the pictures can show — which is stored but not gated until calibrated.
...
### Photo Character
Optional, per-book (child mode) path (#128) that makes the hero **look like a real child**. The parent uploads one photo; a single **Grok-4 vision** call (#397 — Gemini's content filter blocked benign child photos, surfacing as a 503 on upload) checks a child's face is present and extracts an editable **Character Descriptor** (stable facial features only, Russian, for the portrait step and parent editing) plus, since #376, a structured English `Appearance` (`Book.characterAppearance`: skin, hair, the outfit worn in the photo, one detail) that the pages and the judge use with `kind` built from the child's age and gender; the photo is stylised once into a reference portrait (`Book.characterPortraitKey`) that anchors every page, exactly like the synthetic portrait it replaces. The raw upload (`Book.childPhotoKey`) is **transient** — stored privately, deleted the moment book generation starts (see [docs/adr/0006-photo-character-privacy.md]). The stylised portrait is made by the reference-capable provider in use (Grok by default, Gemini fallback).
...
### Visual Bible
The book's visual world (#348), decided ONCE in the **Plan** phase and merged into the persisted `Story` in code (never re-emitted by an LLM): `hero`, `cast` (recurring people/animals besides the hero, ≤3), `locations` (≤3; since #366 structured in the Plan — key object, its **size next to the child**, materials, surroundings — rendered to the descriptor in code, because a peopleless establishing sheet has no other scale anchor), `props` (≤4, each with a Russian `name` since #367) and an `atmosphere` line. Every descriptor is **English, concrete and fixed for the whole book**, reused verbatim on every page so a location or secondary character can no longer drift between spreads. The Plan owns intent↔world coherence: whatever a page intent names must exist in that page's location or props. The **Prose phase sees only the Russian names** — per page a «в кадре» list (place, hero, cast, props) as a constraint, never the English descriptors and never the hero's look — under the rule "unfold with dialogue and feeling, never with new objects, weather or scenery" (#367, the «песок» where the world had only «трава»). Since #360 the Plan emits a **structured appearance** for the hero and every cast member — required `kind`, `skin`, `hair`, `outfit`, `detail` fields — and the descriptor is *rendered* from them in code (`renderAppearance`), so an outfit or a skin tone can never be omitted and the hero's name never enters an image prompt. The hero descriptor is that rendered look, overridden by the parent's appearance or the photo [Character Descriptor] in child mode; the illustration-prompt assembler builds each page prompt deterministically from the bible + the page [Scene] + the page action. Referential integrity is repaired by `normalizeVisualBible`, not by a schema pattern; since #378 repairs are counted **by kind** (dangling location, dropped cast/prop id, cast added from the intent, hero forced) on a `story-plan.normalize` span, so a noisy Plan is measurable, and every model-facing schema field carries a `.describe()` so the model sees what each field holds. The Plan also owns the page count: Prose must return exactly the plan's pages (schema `.length`), and a page whose template differs from the plan gets no scene and fails the structural check into the regeneration loop. The pre-#348 legacy image path (no bible) was removed — a story without a bible cannot be rendered and must be regenerated.
...
### Reference Sheet
A reference image generated once per book to anchor a [Visual Bible] entry across pages (#348, PR 2): one establishing shot per location and one stylised portrait per `cast` member. Passed to the image model as input references (within its reference budget) alongside the hero portrait, so environment and secondary characters stay consistent by picture, not only by text. Always on since ADR-0007's amendment (the flag was removed in #373; without sheets, cast re-drawn from text drifted in outfit and skin tone); S3 keys stored on `Book.referenceImageKeys` and cleaned on book deletion. Cast sheets are the slot a future family-member **photo** portrait drops into unchanged (see the spec's cast → family-photo path).

**Avoid:** "storyboard" — a sheet is a single anchor image, not a page layout.

### Image Eval

One vision-judge verdict on one rendered page for one attempt of one generation **run** (#358, #374) — the image counterpart of [Story Eval]. A retry after `images_failed` is a new run (`run` = 2, …) with its own attempts 1..n, so history is never overwritten and "passed on first attempt" is measured per run; the retry reuses the earlier run's portrait and reference sheets instead of buying them again. `ImageJudgeService` (Grok-4 vision + `generateObject`, #397 — moved off Gemini, whose content filter blocked benign child pages) answers boolean criteria — heroMatch (against the hero portrait), heroOnce, sceneMatch (the page ACTION is what the picture shows), castConsistency / locationConsistency (against the [Reference Sheet]s the page was generated from), proportionsNatural (people and objects in believable proportion to the child; an object the world calls tall reads tall — #366), ageSafe, plus an artefact list (extraLimbs, mergedFaces, textInImage, wrongSurface). A deterministic preflight (bytes, PNG aspect vs template slot) runs first. A failing page is re-rendered (same prompt, fresh sample) up to `IMAGE_EVAL_MAX_RETRIES`; the attempt with the fewest failures ships (soft gate — a judge false negative never blocks a book) and **every attempt writes an `ImageEval` row** and an `image-judge` span — including attempts where the judge gave no verdict: a safety block is recorded as `judge:blocked:<reason>`, any other judge error as `judge:unavailable` (#369) — Grok rarely blocks, so this is mostly dormant now. When the hero portrait is passed as a reference the judge is pointed at it instead of being given the child's text descriptor, which on some pages tripped Gemini's `PROHIBITED_CONTENT` filter (the reason the judge moved to Grok-4 in #397). Always on (the only lever is `IMAGE_EVAL_MAX_RETRIES`, 0 = judge and record, never re-render; #373); calibrated in `docs/process/image-judge-calibration-2026-09-06.md` — recalibrated on Grok-4 (v6, #397): 3/3 bad pages caught, 4/49 false fails (all defensible over-strictness on one split-location story), and — unlike Gemini — 0 pages left unjudged by a safety block. Since #379 every row also carries its **provenance** — the image model, the exact page prompt and the reference labels the page was rendered from — and the book stores the model that rendered its portrait and sheets (`Book.imageModel`), so a bad page is diagnosable from the DB without LangFuse and image cost per provider is derived at read time from `IMAGE_COST_USD` on `/admin/metrics/images` (the vision judge is not priced). Each rendered page also moves the book's SSE progress («Иллюстрация 3 из 8»).
...
### Character Profile
The English visual description of the protagonist in `StorySchema.characterProfile`, used for the synthetic portrait and carried into the hero line of **every** page's illustration prompt and the judge. Since #376 it is **rendered in code from one structured `Appearance`** resolved once after the Plan: in `child` mode the parent's free-text description (when given) is derived into that same schema and wins over the Plan's invented look, and `kind` is built from age + gender; in `observer` mode the Plan's invented look stands. The invariant: portrait prompt, page hero line and judge context are the same words in every mode. When [Photo Character] is used, the approved photo portrait anchors the pages and the photo-derived structured appearance provides the words.
