...
### Волна 8 — Механизация: из прозы в машину (2026-07-16)

**Триггер:** первый систематический аудит харнесса ([`harness-audit-2026-07-16.md`](harness-audit-2026-07-16.md), five-subsystem framework) показал: Scope 5/5, но Lifecycle 2/5 — и начало, и конец сессии держались целиком на дисциплине модели, без единого механизма. Плюс три конкретных дыры: `progress.md` вырос до 127 KB и читался при старте целиком; CI существовал, но не был required check (merge проходил до завершения CI); правило «progress-запись в feature PR» нарушалось (см. friction Волны 7) и никем не проверялось.

**Принцип волны** (согласован с владельцем продукта в дискуссии «не ограничивай модель»): вербальное правило, которое может проверять машина, должно проверяться машиной — тогда оно перестаёт занимать внимание модели вообще. Ограничения-инварианты остаются; ограничения-напоминания механизируются.

| Изменение | Триггер | Зафиксировано |
|---|---|---|
| Ротация `progress.md` (живой лог ≈ последние 10 записей, остальное в архив) | 127 KB × каждая сессия = растущий налог на старт | `progress.md` (rotation policy в шапке), `docs/process/progress-archive-*.md`, AGENTS.md шаг 2 |
| CI (`./init.sh` + PR-title lint) — required check на `main`; merge через `--auto` | Агент self-merge'ил PR, не дожидаясь CI; локальный init.sh — конвенция, не гарантия | Branch protection (GitHub API), `CLAUDE.md` § Git Workflow |
| `SessionStart`-хук: авто-инъекция ориентации (ветка, коммиты, хвост progress, состояние handoff) | Session-start workflow из 6 ручных шагов выполнялся по памяти, частично | `.claude/settings.json` + `scripts/hooks/session-start.sh` |
| Merge-gate: `gh pr merge` блокируется для `issue/*` PR без `progress.md` в диффе | Правило Волны 7 «запись в feature PR» уже нарушалось и не имело проверки | `.claude/settings.json` + `scripts/hooks/pre-merge-gate.py` |
| Правило «≥1 живой `eval:text` до AI-pipeline PR» | Де-факто практика с ADR-0005; unit-тесты промптов — spec-mirroring, качество ловят только живые прогоны | AGENTS.md § Working Rules |
| **Ослаблено:** visual-contract правило для frontend — убраны номера давно закрытых issue, требование смягчено до «проверь мокап, спроси если нет» | Правило ссылалось на закрытые issues #18/#19/#20/… — мёртвый груз | AGENTS.md § Working Rules |

Ослабление зафиксировано по завету раздела «Как продолжать»: правило было полезно, когда UI строился с нуля; после ship'а дизайн-системы (PR #164) оно превратилось в шум.

**Закрытие оставшихся рекомендаций (тот же день):**
- **#162 batch-eval (рек. №4, PR #254):** `pnpm --filter backend eval:batch` — фиксированный сет из 10 кейсов (оба арка × оба режима × возрасты 5–6, включая fallback-эталонные цели), таблица + агрегаты по критериям, `--out=` JSON для before/after. Первый baseline зафиксирован в `docs/process/eval-baselines/2026-07-16-baseline.json`: **10/10 pass, 0 structural-ошибок, registerMatch mean 8.1** — систематическая валидация всех июльских структурных фиксов разом.
- **Гигиена permissions (рек. №6):** `settings.local.json` сокращён **229 → 33** записей — одноразовые захардкоженные команды удалены, оставлены широкие рабочие паттерны (`git *`, `pnpm *`, `gh pr *`, docker/railway read-ops, curl только на localhost/prod-домены). Файл локальный, вне git. Вторая половина рек. №6 (глобальные MCP: computer-use, Figma/Linear не используются в этом проекте, но раздувают стартовый контекст каждой сессии) — решение владельца, т.к. затрагивает все проекты; не выполнено агентом сознательно.

---

### Волна 9 — Внешний аудит через другой агентский движок (2026-07-23)

**Триггер:** владелец продукта прогнал независимый ревью харнесса через Codex (не Claude Code), по шаблону из отдельного проекта (`vibe-engineer 0.3.1`) — полный текст в [`codex-harness-review-2026-07-23.md`](codex-harness-review-2026-07-23.md). Итоговая оценка: **3.7/5** — Instructions 4, State 4, Scope 4.5, Verification **2.5** (боттлнек), Lifecycle 3.5, tool-safety 2. Ключевая рамка отчёта: verification сейчас доказывается прозой в `progress.md` и чекбоксами PR, а не машинно-проверяемой цепочкой `claim → check → run → evidence → proof boundary`.

Самая громкая находка отчёта («`init.sh` упал по всем 6 пунктам в Codex») оказалась при разборе **не багом харнесса, а несовместимостью среды**: Codex-раннер даёт Node 26.5.0/pnpm 11.9.0 против закреплённых в репозитории Node 22.15.0/pnpm 10.29.3 — и pnpm пытается молча пересобрать `node_modules` под чужой тулчейн, зависая без TTY. Важное уточнение, которое выяснилось только при попытке пофиксить: **эта же машина под Claude Code тоже даёт Node 26.5.0**, а не 22.15.0 из `.nvmrc` — и вся сессия весь день до этого прошла нормально. Первая версия фикса (жёсткий fail при любом расхождении с `.nvmrc`) была бы регрессией — заблокировала бы каждую будущую сессию на этой же машине без реальной проблемы. Настоящий контракт версии — `engines.node` (`>=22.0.0`) в package.json, а не точный пин `.nvmrc` (dev-convenience для nvm/fnm). Это конкретный пример того, зачем нужна Фаза 1 систематического дебага («сначала гипотеза, потом тест») даже когда чинишь харнесс, а не продуктовый код.

**Принцип волны:** внешний ревью другим агентским движком — ценный источник находок именно потому, что видит харнесс без слепых пятен, накопленных за 8 предыдущих волн одним и тем же движком. Но находки нужно **верифицировать против фактического поведения**, а не применять буквально — иначе можно "починить" то, что не было сломано, и сломать то, что было.

| Изменение | Триггер | Зафиксировано |
|---|---|---|
| `init.sh`: preflight Node (диапазон `engines.node`, не точный `.nvmrc`) + WARN (не fail) на несовпадение pnpm с `packageManager` | П0 отчёта — но скорректировано после того, как жёсткий fail на Node сломал бы текущую рабочую машину | `init.sh` |
| `init.sh`: убран `pnpm --silent`; добавлен `format:check` | `--silent` прятал ровно ту диагностику, которая нужна при расхождении версий; `format:check` не входил в gate вообще | `init.sh` |
| Найдено фиксом `format:check`: устаревший каталог `backend/generated/prisma` (до смены `output` в schema.prisma), не используется кодом, не в git — удалён; актуальный `backend/src/generated` добавлен в `.prettierignore` | Первый же прогон нового `format:check`-шага | `.prettierignore` |
| Разведены два вида evidence для AI-pipeline фич: **prose-quality** (`eval:text`/`eval:batch`, без записи в БД, `bookId: "dry-run"`) vs **product-integration** (реальная генерация через API, настоящая `StoryEval` row) — раньше «Done is not a mood» требовал одновременно LangFuse-трейс и `StoryEval` row, а единственное явно указанное средство проверки (`eval:text`) само по себе не может дать `StoryEval` row | Отчёт, находка №4 (внутреннее противоречие AI evidence) | `AGENTS.md` § Working Rules и § Agent Behavior Contract, `CLAUDE.md` § Definition of Done |
| Убран week-milestone-first выбор issue при старте сессии, заменён на `ready-for-agent` + приоритет + явные блокеры | Отчёт, находка №8 — проект давно перерос 5-недельный roadmap, у реальных открытых issues (#155, #157) нет milestone | `AGENTS.md` § Session Start Workflow |

**Что из отчёта осознанно НЕ сделано сейчас (не потому что неверно, а по времени — до защиты 2026-07-27):**
- Полная JSON-схема Evidence Packet (git SHA, environment, exit codes, trace ID, proof boundary на каждый feature/release) — идея здравая, но объём внедрения не оправдан оставшимся временем. Кандидат на Волну 10, если харнесс проживёт после защиты.
- Разделение portable core / Claude-specific adapters (hooks, skill map) — реальный пробел (skill-инструкции этого харнесса не работают в Codex), но переписывать под мульти-движок сейчас — не приоритет: проект фактически ведётся одним движком.
- Пересборка AI-baseline метаданных (git SHA, точные model ID вместо `"default"`, digest промпта, verdict против прошлого baseline) — ценно для долгосрочной воспроизводимости, не блокирует защиту.
- Дальнейшее сужение permissions в `settings.local.json` — локальный файл, не в git, низкий риск.
- `verify:release`/`verify:ai` как отдельные именованные профили (а не `init.sh` + `verify.sh`, уже разделённые по факту в #155) — семантически то же самое другими именами; не переименовывалось, чтобы не плодить документацию без функциональной разницы.

Отчёт целиком сохранён в [`codex-harness-review-2026-07-23.md`](codex-harness-review-2026-07-23.md), с примечаниями прямо в тексте о том, что применено, что скорректировано и что отложено.
