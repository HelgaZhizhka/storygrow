...
## (опц.) Фото-персонаж (~40 s) — вставлять ТОЛЬКО при запасе времени

> ⚠️ Живой апрод рискован: генерация портрета — это вызов Gemini, а он сегодня
> уже упирался в месячный лимит расходов (429). Безопасный вариант — показать
> **заранее сгенерированную** книгу-по-фото (см. `staged-books.md`), а не
> загружать фото вживую. Живой апрод — только если уверены в квоте.

**Безопасный путь:** открыть готовую книгу-по-фото → показать 2–3 страницы:

> «Вот книга, где герой нарисован по загруженному фото ребёнка — узнаётся на
> всех страницах. Родитель сначала видит превью портрета с редактируемым
> описанием и подтверждает его, и только потом генерируется книга. Сырое фото
> удаляется сразу при старте генерации — остаётся только рисунок.»

**Живой путь (если уверены в квоте):** Новая книга → Custom → Ребёнок-герой →
галочка «Сделать героя похожим на моего ребёнка» → шаг портрета → загрузить
фото + согласие → превью → «Создать книгу».
...
User → form
  → BullMQ job
    ...
    4. ImageGenerator   (Gemini 2.5 Flash Image + портрет-референс из текста ИЛИ фото ребёнка → один герой; gpt-image-1 fallback)
...
> «А это уже отгружено — прямо к защите: фото ребёнка как основа героя. Родитель загружает фото, и герой книги рисуется похожим на него на всех страницах. Приватность решена честным тонким срезом — явное согласие, сырое фото удаляется сразу при старте генерации, и Gemini вместо более сильной Alibaba Qwen из-за трансграничной биометрии ребёнка; «известные пробелы» для массового прода задокументированы в ADR-0006.»
