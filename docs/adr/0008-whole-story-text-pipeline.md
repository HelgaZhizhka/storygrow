...
**4. Length, language and text preservation are deterministic gates.** Word range per band, Russian-only text, layout feasibility of the finished text, split legality and exact reconstruction are checked in code; a length miss or an unplaceable sentence gets a targeted revision inside the existing retry budget. This gives ADR-0005's optional Edit pass its concrete job. The concrete revision policy is a proposal (spec §6), not a product rule.
...
| No new UI mode for independent readers | PDF capacity lever (pages vs. caps vs. template vs. ranges) — on rendered samples |
...
- **Images and PDF are untouched** in contract; PDF *capacity* is a real conflict (spec §7, estimates) resolved by the owner on rendered samples with readable type, never by cutting text or shrinking type. Rendering the parent note, if chosen, changes the `final` template — a later decision.
