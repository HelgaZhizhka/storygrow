...
Input: goal (catalogue or custom) · child age → age band · arc · protagonist mode ·
  ...
  7. PDF               templates, caps and page counts chosen on rendered samples
                       (stage 1b); never truncated, never shrunk
```
...
**Deterministic gates (code, before any model-based check).** Word count inside the band range (one counting rule, the pilot's — no standalone dashes — recorded in code); Russian-only text (existing `checkLanguagePurity`); title gate (`isConcreteTitle`); **layout feasibility** — against the *chosen* layout (after stage 1b), the longest sentence must fit the largest page cap, checked here so an unplaceable sentence is caught before a paid split call; not applied while no layout is chosen, and never a reason to narrow word ranges [proposal; a check on the finished text, not an authoring instruction]; for flaw arcs at least one character besides the hero [proposal].
...
**Revision policy (bounded) [proposal].** `EVAL_MAX_RETRIES` stays the single budget, owned by the orchestrator. A length miss triggers a **targeted revision** of the same tale ("shorten to N–M words; keep the events, the characters and the consequences; cut description and repetition" — dialogue may be trimmed; nothing here is a product rule); a safety `fail` or a language failure triggers a **fresh tale** with feedback. **No judge criterion — causality included — triggers a regeneration while the judge is informational**: its findings are recorded in `StoryEval.judgeScores` and read by a human. Automatic regeneration on a craft criterion is a *conditional later switch*, allowed only after stage 3 has shown the judge agrees with the owner on new results; it is not a rule of this design. A failed book check (4b) triggers one re-split, not a new attempt. Every attempt writes exactly one `StoryEval` row when it ends (hard constraint 10); rows are never overwritten (no silent regeneration).

**A revision is a new text version.** The earlier version stays in the attempt record (its `StoryEval` row and trace); the new version goes through **every** text check again — deterministic gates, safety gate, judge — before the split, scenes and bible are derived from it, and the reconstruction check in 4b compares the pages against this new accepted version, never the old one. The "split this unplaceable sentence" edit is the same mechanism used as a **rare fallback** for an outlier; it is not the way to keep old caps. A text that exceeds the book's total capacity is a layout question (stage 1b) or, if reading says the band wants shorter tales, a word-range question — sentence splitting fixes neither, and word ranges are **never narrowed to fit the old templates**.
...
| Prompts | `backend/src/ai/prompts/plan.prompt.ts`, `prose.prompt.ts`, `story-generator.prompt.ts` (beat sheets) | **Kept intact behind the `TEXT_PIPELINE` flag for the whole rollback window** (stage 6: one release after the default flips) — prompts, schemas and their dependencies; deletion is a separate later step once rollback is no longer needed. The beat sheets' per-arc meaning is reused in the brief. |
...
| Schemas | `backend/src/ai/schemas/story-plan.schema.ts` | Kept with the Plan path for the rollback window; removed only in the later deletion step. |
| | new `whole-story.schema.ts`, `page-split.schema.ts`, `story-brief.schema.ts`, `story-safety.schema.ts` | Stage contracts (§4, §6). |
| | `story.schema.ts` | Optional `wholeText` (immutable source), `parentNote` (storage only); `buildProseSchema` stays as long as the Plan path exists. |
...
| PDF | `backend/src/pdf/page-templates/page-templates.config.ts`, `final.html` (only if the parent note is rendered there) | Templates (HTML/CSS), `suitableFor`, per-band caps and `PAGE_COUNT_BY_BAND` are **expected to change** — today's 110/220 caps and page counts are not requirements of the new path. `pdf-render.service.ts` stays only if the layout comparison (below) confirms compatibility; a changed `maxChars` number alone proves nothing about physical fit. |
...
**PDF capacity — a real conflict; the numbers are estimates.** 5–6 text pages cap at 220 chars (`image-top/bottom`), the final page at 200, and a book has at most 12 pages, so the ceiling is ~2 400 chars ≈ **340–370 words** at ~6.5 chars/word — the *bottom* of the 350–550 range, reachable only at the maximum page count; 450 words needs ~14–15 text pages at today's caps. For 3–4 (110/90 chars, 6–8 pages) the ceiling is ~750 chars ≈ **100–115 words** against a 150–250 target. Levers: more text area per page at a comfortable type size, more pages per band, a text-heavier 5–6 template (`text-focus` is 350 chars but `suitableFor` 7–8 only). Narrowing the word ranges is **not** a lever: volume is settled by reading, capacity by rendering. Chars-per-word is approximate and a larger cap does not prove physical fit: the decision is taken on **rendered samples with readable type, after an accepted text exists** (stage 1 is not blocked by it). Until then the book check refuses a split that does not fit.

**Layout comparison (stage 1b) [proposal — the protocol; the design itself is the owner's decision after samples].** On the **same** accepted 5–6/virtue text, prepare two comparable PDF samples: (a) more text area per page at a comfortable type size; (b) more pages with the large illustrations kept. Use the same existing illustrations or clearly marked placeholders — **no paid image generation** just to compare layouts. Show the owner the real pages, the total page count, readability, and the text/image ratio; if a variant changes the number of illustrations, assess its cost and generation-time impact separately. After the choice, check the layout on other texts, including the upper word bound and long sentences or dialogue; the 3–4 band gets its own check on an accepted 3–4 text (stage 2). Layout may differ per age band under one literary style. The local review's starting preference — try more text area for 5–6 before adding illustrated pages — is a comparison order, not a design decision.
...
| 1b | **Layout comparison.** Two rendered samples on the stage-1 text per the §7 protocol; existing illustrations or placeholders, no paid image runs. Owner chooses; the chosen layout's caps, page counts and `suitableFor` become the numbers the feasibility gate and the book check use. | A chosen layout with rendered evidence; checked on ≥ 2 more texts incl. the upper word bound. | 1 |
...
| 3 | **New goals and repeats — release gate.** Portability matrix: 9 unedited results, 3 per combination, including ≥ 2 custom goals and one goal repeated for the same child; matrix and criteria fixed before the run. Judge v2 agreement re-measured on these nine. | Read by the owner; diversity, language and judge-agreement findings written down and addressed. Not a statistical proof, but **a precondition for release**. Includes an explicit **decision on what gates text quality after release**, when no human reads each book: either the judge criteria that proved reliable here (causality, goal/arc) are switched on as gates, or the owner knowingly accepts release with safety + deterministic gates only. | 2 |
| 4 | **Integration (technical readiness, not release readiness).** Split + book check + bible + title behind `TEXT_PIPELINE=whole`; real API run → Book, StoryEval, own trace, images, PDF viewed. | One real book end-to-end, evidence attached. | 1 (scaffold), 1b (any PDF claim), 2 (merge) |
| 5 | **Real books — all three combinations.** Owner generates books in the app for 3–4/virtue, 5–6/virtue and 5–6/flaw; chosen layout applied, with the 3–4 layout checked on that band's accepted text; split fidelity, image continuity and PDF checked on real output. | Findings for each combination; no silent cuts. | 1b, 3, 4 |
| 6 | **Release with rollback.** Flag default → `whole`; the Plan path stays one release; rollback = flip the flag. | Released; the Plan path stays intact for one release and is removed in a separate later step. | 3, 5, STO-6 |
...
- **Split fidelity.** Mitigated by construction (verbatim extraction at allowed boundaries, reconstruction assertion); residual risk is a single sentence longer than any page of the chosen layout → caught by the feasibility gate, handled by the rare fallback revision, never by cutting; if it stops being rare, the layout is wrong, not the text.
...
- **Capacity.** §7 numbers are estimates; decided on rendered samples (stage 1b), never by shrinking type, cutting text or narrowing word ranges to fit old templates.
...
1. **Layout** (§7, stage 1b): between the two rendered samples on an accepted text — more text area at a comfortable type size, or more pages with large illustrations — possibly different per band. Word ranges are not on the table as a lever.
2. **Parent note placement**: with the discussion questions on the `final` page, or elsewhere — decided together with the layout, since both touch the same template.
