...
- New CI job `verify` in `.github/workflows/ci.yml`, triggered only on `push: main`, using GitHub Actions' native Postgres `services:` block (avoids a port clash with `verify.sh`'s own `docker compose up` for redis/minio) and dummy config values for providers the fast-flow path never calls, plus the one real secret it needs.
